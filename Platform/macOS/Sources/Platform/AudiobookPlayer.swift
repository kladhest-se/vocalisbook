...
public final class AudiobookPlayer {
    ...
    /// media keys cannot: they are `MPRemoteCommandCenter` targets that call this
    /// type directly and know nothing about the app's modes.
    ///
    /// So the gate lives here, where every path passes through.
    ...
    /// Nothing to observe.
    ///
    /// There is no `AVAudioSession` on macOS, so there are no interruption or
    /// route-change notifications: no phone call to duck for, and unplugging
    /// headphones is handled by the system rather than reported to the app.
    /// The method is kept so the shape of this class matches the other ports and
    /// a reader comparing them is not left wondering where it went.
