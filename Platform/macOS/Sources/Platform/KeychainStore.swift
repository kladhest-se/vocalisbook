import Foundation
// `sysctlbyname`, for the hardware model. Foundation re-exports Darwin on this
// platform, but relying on a re-export for a C function is how a build breaks
// when an unrelated import is tidied.
import Darwin
import Security
import AppKit
import PlexKit
...
public enum DeviceIdentity {
    ...
    public static func current(store: KeychainStore, version: String) -> PlexClientIdentity {
        ...
        // There is no UIDevice here. The computer name is what Plex shows in its
        // authorised-devices list, so it is worth getting right — but it is also
        // user-editable, which is exactly why the client identifier above is not
        // derived from it.
        let os = ProcessInfo.processInfo.operatingSystemVersion

        // The model, as the other two ports send theirs.
        //
        // This said "Mac" flatly, while iOS and tvOS send `device.model` — so a
        // dashboard listing three of this app's clients showed two models and
        // one word. `hw.model` is the identifier a Mac reports for itself,
        // which is what `UIDevice.model` is on the other side.
        var size = 0
        sysctlbyname("hw.model", nil, &size, nil, 0)
        var model = [UInt8](repeating: 0, count: max(size, 1))
        sysctlbyname("hw.model", &model, &size, nil, 0)

        // Decoded rather than read as a C string.
        //
        // `String(cString:)` is deprecated, and its replacement wants the bytes
        // without the terminator — which `sysctl` includes in the length it
        // reports, so dropping the last byte is part of the conversion rather
        // than an optimisation.
        let hardware = String(decoding: model.prefix(while: { $0 != 0 }), as: UTF8.self)

        return PlexClientIdentity(
            clientIdentifier: identifier,
            product: "VocalisBook",
            version: version,
            device: hardware.isEmpty ? "Mac" : hardware,
            // The computer's name, which is what Plex lists under authorised
            // devices — and what a dashboard shows as the player. If that reads
            // as a person rather than a machine, it is the name in Sharing
            // settings rather than anything this app chose.
            deviceName: Host.current().localizedName ?? "Mac",
            platform: "macOS",
            platformVersion: "\(os.majorVersion).\(os.minorVersion).\(os.patchVersion)"
        )
    }
}
