...
/// Wires Control Centre, the Now Playing widget and the hardware media keys to
/// the player.
///
/// This is all of the media key handling there is on macOS. `MPRemoteCommandCenter`
/// covers the keys and the Now Playing widget together; a global event tap would
/// need the Accessibility permission and would get the app rejected. Menu
/// commands and their keyboard shortcuts are separate, and live in
/// `PlaybackCommands` in the app target.
///
/// Everything reported here is book-level, not track-level: the scrubber shows
/// position within the whole book, which is the only figure a
