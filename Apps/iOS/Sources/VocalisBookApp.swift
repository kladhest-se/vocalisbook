...
struct VocalisBookApp: App {
    ...
    var body: some Scene {
        WindowGroup {
            RootView()
                .environment(app)
                // Applied once, at the root. A theme that has to be remembered
                // at every call site is a theme that will be forgotten at one.
                .themed(app.themes.current)
                // One `start`, and the pending intent right after it.
                //
                // A second `.task { await app.start() }` alongside this would
                // run at the same time — signing in twice and syncing the
                // library twice on every launch — because `.task` modifiers all
                // fire when the view appears, not in sequence.
                .task {
                    await app.start()
                    app.honourPendingIntent()
                }
                .onChange(of: systemScheme, initial: true) { _, scheme in
                    app.themes.systemIsDark = scheme == .dark
                }
                .task { await app.trackDaylight() }
                // And again on return, because the app may already have been
                // running when Siri was asked.
                .onChange(of: scenePhase) { _, phase in
                    guard phase == .active else { return }
                    app.honourPendingIntent()
                    // Anything listened to while this was in the background, or
                    // while there was no network, goes out now.
                    app.catchUp()
                }
        }
    }
...
final class AppModel {
    ...
    func select(section: PlexLibrarySection, serverIdentifier: String) {
        ...
        // after launch and usually before any of this: both would find no
        // library and do nothing, and the first thing somebody saw would be
        // yesterday's list.
        refreshActiveBooks()
    }
    ...
    /// The name of the server, for the settings screen.
    ...
    /// Keeps `isAfterDark` roughly right for the night-mode setting.
    ///
    /// Deliberately a clock rather than CoreLocation: knowing the actual sunset
    /// would need the user's position, and asking for that to pick a colour
    /// scheme is not a trade worth offering. Between 20:00 and 07:00 is close
    /// enough for something whose only consequence is a dimmer palette.
    func trackDaylight() async {
        while !Task.isCancelled {
            let hour = Calendar.current.component(.hour, from: Date())
            themes.isAfterDark = hour >= 20 || hour < 7
    ...
    /// A book some other screen wants opened.
    ///
    /// The downloads list lives inside the Settings sheet, and a book opened
    /// from there was pushed *into* that sheet — the reader appearing inside a
    /// modal called Settings, with Done as the way out. It worked, and it was
    /// clearly the wrong place to end up.
    ///
    /// Set this instead: the sheet closes, Home comes forward, and the book
    /// opens where books open. The Mac has carried the same channel since its
    /// detail pane needed one; this is the phone's.
    ///
    /// Cleared once honoured, so asking twice for the same book works.
    var requestedBook: String?

    /// Does whatever Siri asked for, if anything.
    ///
    /// Called after `start()` and again when the app becomes active, because the
    /// app may have been running already when the phrase was spoken.
    ///
    /// Cleared as it is read, so a request is acted on once. Leaving it set
    /// would resume the book again every time the app came forward, which is a
    /// strange thing for an audiobook to do at a traffic light.
    func honourPendingIntent() {
        guard let request = IntentRequests.shared.pending else { return }
        IntentRequests.shared.pending = nil
    ...
    /// Resumes what was last listened to, or opens it if it cannot be played.
    ///
    /// If the book is already loaded, this is a play. If it is not — the app was
    /// launched by the intent — the book screen has the loading in it, so the
    /// request becomes "open this" and the person presses play. Better than a
    /// silent failure, and better than duplicating the load here.
    private func resumeLastBook() {
        if player.bookRatingKey != nil {
            player.play()
            return
        }

        guard let library,
              let recent = try? library.continueListening(limit: 1, downloadedOnly: isOffline),
              let book = recent.first
        else { return }

        open(bookRatingKey: book.ratingKey)
    }

    /// Which books are fully downloaded, for the badge on a cover.
    ///
    /// Held here rather than passed to every grid: tiles are drawn from six
    /// screens, and a parameter would have to be threaded through all of them
    /// and remembered at each new one. A tile already reads this model.
    ///
    /// Refreshed when downloads change and at launch, not per tile — the query
    /// is one round trip and a grid draws hundreds of covers.
    ...
    func open(bookRatingKey: String) {
